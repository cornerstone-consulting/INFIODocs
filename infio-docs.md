...
- **Amazon VPC (Core)**: Provides the underlying network infrastructure to securely connect EC2 instances and other AWS services such as S3, KMS, and Secrets Manager, etc.
- **Amazon EC2 (Core)**: Provides scalable compute resources for running INFIO tool or services within the VPC. 
- **Amazon S3 (Core)**: Serves as a storage solution for database assessment artifacts, including DDL files, DMS Assessor reports, Object Dependency files, Profiler event logs, and database migration assessment reports generated by the INFIO tool.
...
- **AWS IAM Role (Supporting)**: Grants permissions to INFIO EC2 and AWS DMS to access required AWS services securely. This includes the IAM role assigned to the INFIO EC2 instance and the DMS instance profile, which enables AWS DMS to interact with database resources..
- **AWS DMS Instance Profile (Supporting)**: Facilitates the necessary permissions for AWS DMS to connect with source and target databases, ensuring generate schema conversion report process.
- **AWS DMS (Core)**: Generates the AWS DMS Schema Conversion report, providing insights into database schema compatibility and necessary modifications for migration.
...
| **AWS DMS Service**  | Migration project  | $0.00  | 
...
The INFIO EC2 instance is deployed within an Amazon Virtual Private Cloud (VPC) to provide a secure and isolated network environment. The VPC ensures that INFIO resources remain protected from unauthorized access.
The VPC is designed to control network traffic, ensuring that the INFIO EC2 instance can securely communicate with other AWS services by placing it in private or public subnet.
...
#### Security groups
Security Groups act as a virtual firewall for controlling inbound and outbound traffic to the INFIO EC2 instance. Only essential ports are opened to restrict unnecessary access. Security Groups providing another layer of network security beyond IAM and VPC controls.
...
#### 2. Verify IAM Role and Network Connectivity for INFIO EC2 Instance

Before proceeding, ensure that the INFIO EC2 instance has the correct IAM role attached that allows connectivity to essential AWS services, including S3, KMS, DMS, Secrets Manager, EC2, IAM, CloudFormation, and VPC. If your network does not have full connectivity, consider deploying VPC Endpoints for private access or configuring a NAT Gateway/Internet Gateway for external access. Missing permissions or restricted network access may lead to deployment failures or operational issues.
...
> **Note on Deployment Time**: The deployment time for INFIO EC2 instance will be around more than one hour at most, enabling user to move forward with INFIO tool efficiently.

> **Important Note for INFIO EC2 login password**: When the INFIO EC2 instance is deployed, a PEM key will be automatically generated. Download PEM key from aws parameter store to retrieve the Administrator password for the INFIO EC2 instance. You will need the PEM key to decrypt the password in order to RDP or connect to INFIO EC2 instance.
